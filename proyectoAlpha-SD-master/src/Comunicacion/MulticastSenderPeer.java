...
public class MulticastSenderPeer implements Compute {
        ...
        private static final String dirMulti = "229.5.6.7";
        ...
        public static void runRMIServer(){
            try {
                ...
                System.setProperty("java.security.policy","file:\\C:\\Users\\hca.ITAM\\Desktop\\proyectoAlpha-SD-master\\src\\RMI\\client.policy");
	...
	MulticastSocket s =null;
   	 ...
   	 try {
                ...
                InetAddress group = InetAddress.getByName(dirMulti); // destination multicast group 
	   	...
	   	s.joinGroup(group);
                ...
                while(t)
                {
                    if(System.currentTimeMillis()-time>=10000 || altcont==0){
                        ...
                        if(c!=null && !c.clientSocket.isClosed())
                            c.clientSocket.close();
                        
                        time = System.currentTimeMillis();
                        int temp = (int)Math.round(Math.random()*10+1);
                        ...
                        try{                            
                            ...
                            c = new Connection(clientSocket); //Es un hilo
                            
                            String data = c.correr();
                            
                            int tempo = clientSocket.getRemoteSocketAddress().toString().indexOf(":");
                            System.out.println("ip: " + clientSocket.getRemoteSocketAddress().toString().substring(1,tempo));
                            int index = ips.indexOf(clientSocket.getRemoteSocketAddress().toString().substring(1, tempo));
